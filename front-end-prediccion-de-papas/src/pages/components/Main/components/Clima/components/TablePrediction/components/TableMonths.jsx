...
export const TableMonths = ({ dataMeses, nombre }) => {
  const dataConFechas = dataMeses.map((item, index) => {
    return {
      ...item,
  ...
  // console.log("la ultima fecha es : "   + ultimaFecha)
  ...
  return (
    <div className="flex-1 p-4 ">
      <div className="flex justify-between" >
        <h2 className="text-lg font-bold mb-2">Prediccion - {nombre} </h2>
        <DownloadPDF climateData={dataMeses} />
      </div>
              ...
              <tr key={index}>
                ...
                <td className="text-black">
                  {dataForMonth ? (
                    ...
                    <p className="bg-slate-200 py-1 px-2  rounded-md w-max font-medium">
                      Desconocido{" "}
                    </p>
                  )}
                </td>
